...
app.get('/pay', async (req, res) => {
    try {
        ...
        return res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Preparing Payment</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);min-height:100vh;display:flex;align-items:center;justify-content:center;color:white}.container{background:rgba(255,255,255,0.1);backdrop-filter:blur(10px);padding:40px;border-radius:15px;max-width:500px}.loader{margin:20px auto;border:4px solid rgba(255,255,255,0.3);border-top:4px solid #fff;border-radius:50%;width:50px;height:50px;animation:spin 1s linear infinite}.btn{background:white;color:#333;padding:12px 24px;text-decoration:none;border-radius:8px;margin-top:20px;font-weight:600;display:inline-block}@keyframes spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}</style></head><body><div class="container"><h2>Preparing your payment link...</h2><div class="loader"></div><p>Order ${latestOrder.name} is being processed</p><div id="countdown">Checking in 2 seconds...</div><a href="https://${SHOPIFY_STORE_DOMAIN}" class="btn">Back to Store</a></div><script>let attempt=0;let countdown=2;const countdownEl=document.getElementById('countdown');function updateCountdown(){if(countdown>0){countdownEl.textContent='Checking in '+countdown+' seconds...';countdown--}else{countdownEl.textContent='Checking now...'}}function checkPayment(){attempt++;fetch('/check-payment-link').then(res=>res.json()).then(data=>{if(data.ready&&data.payUrl){window.location.href=data.payUrl}else if(attempt<10){setTimeout(checkPayment,2000);countdown=2;updateCountdown()}else{countdownEl.textContent='Taking longer than expected...';setTimeout(()=>window.location.reload(),3000)}}).catch(()=>{if(attempt<10){setTimeout(checkPayment,2000);countdown=2;updateCountdown()}})}const countInterval=setInterval(()=>{updateCountdown();if(countdown<0){clearInterval(countInterval);checkPayment()}},1000);updateCountdown()</script></body></html>`);
